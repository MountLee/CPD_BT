...
class online_cpd_cv():
    def __init__(self, step, buffer, gamma_list, lam_list):
#         self.start = start
#         self.end = end
        self.step = step
        self.buffer = buffer
    ...
    def estimate_func(self, Y, X, lam = None):
        pass

    def joint_estimate_func(self, Y1, X1, Y2, X2, lam = None):
        pass
    ...
    def screen_cp(self, data, st, ed, step, threshold, lam = None):
        Y, X = data
        buffer = self.buffer
        step = self.step
        
        if st + 2 * buffer + step >= ed:
            return None
        n_step = (ed - st - 2 * buffer - 1) // step + 1

        theta_joint = self.estimate_func(Y[st:ed], X[st:ed], lam)
        loss_joint = self.loss_func(Y[st:ed], X[st:ed], theta_joint)
        
        loss_min = np.infty
        loc_min = None
        for i in range(1, n_step):
            loc = st + buffer + int(i * step)
            ...
            if loss_split < loss_min:
                loss_min = loss_split
                loc_min = st + buffer + int(i * step)

        if loss_min + threshold < loss_joint:
            return loc_min
        else:
            return None
    
    def online_cpd(self, data, start, end, step, gamma, lam = None):
        if 2 * step > end - start:
            return []
        ...
        cp_list = []
        last_end = start
        while last_end + 2 * step < end:
            cur_start = last_end
            cur_end = cur_start + 2 * step

            while cur_end <= end:
                loc = self.screen_cp(data, cur_start, cur_end, step, gamma, lam)
                if loc:
                    cp_list.append(loc)
                    last_end = loc
                    break
    ...
    def fit(self, data_train, data_test, start, end):
    ...
    #     X_test, Y_test = data_test
        ...
        for i, x in enumerate(lam_gamma_list):
            lam, gamma = x
            cp_loc = self.online_cpd(data_train, start, end, self.step, gamma, lam)
...
########## bt specialized functions


class online_cpd_cv_bt(online_cpd_cv):
    def __init__(self, step, buffer, gamma_list, lam_list):
#         self.start = start
#         self.end = end
        self.step = step
...
##### Borda count + ranking distance


class wbs_cv_borda_rank(wbs_cv):
    def __init__(self, m_intervals, threshold_list, grid_n, smooth = 10, buffer = 10):    
        """
        smooth: length of smoothing window for cusum_func
        buffer: buffer for binary segmentation, when segmenting at bm, search (start, bm - buffer], (bm + buffer, end] 
    ...
    def cusum_func(self, Y, grid, sm, em, smooth = 1):
        ...
        borda_left[0] = np.sum(y_train[:grid_train[0] + 1], axis = 0)
        borda_right[nt - 1] = np.sum(y_train[grid_train[nt - 1] + 1:], axis = 0)

        rank_left, rank_right = np.zeros((nt, n)), np.zeros((nt, n))

        rank_left[0] = stats.rankdata(borda_left[0], method = 'min')
        rank_right[nt - 1] = stats.rankdata(borda_right[nt - 1], method = 'min')

        diff = np.zeros(nt - 1)

        for i in range(1, nt):
            lf = np.sum(y_train[grid_train[i - 1] + 1:grid_train[i] + 1], axis = 0)
            borda_left[i] = borda_left[i - 1] + lf
            rt = np.sum(y_train[grid_train[nt - 1 - i] + 1:grid_train[nt - 1 - (i - 1)] + 1], axis = 0)
            borda_right[nt - 1 - i] = borda_right[nt - 1 - (i - 1)] + rt
        ...
        for i in range(smooth, nt - smooth):
            ...
            diff[i] = self.kendall(rank_left[i], rank_right[i])
    ...
    def goodness_of_fit(self, Y_train, Y_test, cp_loc):
        ...
        for i in range(len(cp_loc) - 1):
            if cp_loc[i + 1] > cp_loc[i] + 1:
                borda_train = Y_train[cp_loc[i]:cp_loc[i + 1]].sum(axis = 0)
                rank_train = stats.rankdata(borda_train, method = 'min').reshape((-1,1))

                match = np.matmul(Y_test[cp_loc[i]:cp_loc[i + 1]], rank_train).squeeze()
                res.append(np.sum(match < 0))
    #         else:
    # #             res += gamma
    #             res += 0
        return np.mean(res)
